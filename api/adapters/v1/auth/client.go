// Package auth is the SDK adapter for komodo-auth-api.
// Types in this file are derived from komodo-auth-api/docs/openapi.yaml.
//
...
// Update this when komodo-auth-api ships a new major version and the typed
// surface here is verified against it.
var supportedVersions = map[int]struct{}{
	1: {},
}

// Client is the SDK adapter for komodo-auth-api.
type Client struct {
	baseURL string
	version int
	http    *httpc.Client
...
func NewClient(baseURL string, ver int) (*Client, error) {
	...
	if _, ok := supportedVersions[ver]; !ok {
		return nil, fmt.Errorf("auth: unsupported api version %d", ver)
	}
	return &Client{
		baseURL: baseURL,
		version: ver,
