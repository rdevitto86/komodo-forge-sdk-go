// Package cart is the SDK adapter for komodo-cart-api.
// Types in this file are derived from komodo-cart-api/docs/openapi.yaml.
//
...
// Update this when komodo-cart-api ships a new major version and the typed
// surface here is verified against it.
var supportedVersions = map[int]struct{}{
	1: {},
}

// Client is the SDK adapter for komodo-cart-api.
type Client struct {
	baseURL string
	version int
	http    *httpc.Client
...
func NewClient(baseURL string, ver int) (*Client, error) {
	if baseURL == "" {
		return nil, fmt.Errorf("cart: baseURL is required")
	}
	if _, ok := supportedVersions[ver]; !ok {
		return nil, fmt.Errorf("cart: unsupported api version %d", ver)
	}
	return &Client{
		baseURL: baseURL,
		version: ver,
