// Package orderreservations is the SDK adapter for komodo-order-reservations-api.
// Types in this file are derived from komodo-order-reservations-api/docs/openapi.yaml.
//
...
// Update this when komodo-order-reservations-api ships a new major version and the
// typed surface here is verified against it.
var supportedVersions = map[int]struct{}{
	1: {},
}

// Client is the SDK adapter for komodo-order-reservations-api.
type Client struct {
	baseURL string
	version int
	http    *httpc.Client
...
func NewClient(baseURL string, ver int) (*Client, error) {
	if baseURL == "" {
		return nil, fmt.Errorf("order-reservations: baseURL is required")
	}
	if _, ok := supportedVersions[ver]; !ok {
		return nil, fmt.Errorf("order-reservations: unsupported api version %d", ver)
	}
	return &Client{
		baseURL: baseURL,
		version: ver,
