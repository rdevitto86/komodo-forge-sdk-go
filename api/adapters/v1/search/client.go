// Package search is the SDK adapter for komodo-search-api.
// Types in this file are derived from komodo-search-api/docs/openapi.yaml.
//
...
// Update this when komodo-search-api ships a new major version and the typed
// surface here is verified against it.
var supportedVersions = map[int]struct{}{
	1: {},
}

// Client is the SDK adapter for komodo-search-api.
type Client struct {
	baseURL string
	version int
	http    *httpc.Client
...
func NewClient(baseURL string, ver int) (*Client, error) {
	if baseURL == "" {
		return nil, fmt.Errorf("search: baseURL is required")
	}
	if _, ok := supportedVersions[ver]; !ok {
		return nil, fmt.Errorf("search: unsupported api version %d", ver)
	}
	return &Client{
		baseURL: baseURL,
		version: ver,
