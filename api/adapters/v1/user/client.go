// Package user is the SDK adapter for komodo-user-api.
// Types in this file are derived from komodo-user-api/docs/openapi.yaml.
//
...
// Update this when komodo-user-api ships a new major version and the typed
// surface here is verified against it.
var supportedVersions = map[int]struct{}{
	1: {},
}

// Client is the SDK adapter for komodo-user-api.
type Client struct {
	baseURL string
	version int
	http    *httpc.Client
...
func NewClient(baseURL string, ver int) (*Client, error) {
	if baseURL == "" {
		return nil, fmt.Errorf("user: baseURL is required")
	}
	if _, ok := supportedVersions[ver]; !ok {
		return nil, fmt.Errorf("user: unsupported api version %d", ver)
	}
	return &Client{
		baseURL: baseURL,
		version: ver,
