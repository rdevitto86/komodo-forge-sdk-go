...
// Publishes Events to an SNS FIFO topic.
type Publisher struct {
	sns      sns.API
	topicARN string
}

// Creates a Publisher that publishes to the given SNS topic ARN.
...
func (p *Publisher) Publish(ctx context.Context, event Event) error {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("failed to publish event %s (%s)", event.ID, event.Type), err)
		return err
	}

	logger.Info(fmt.Sprintf("published %s id=%s entity=%s", event.Type, event.ID, event.EntityID))
	return nil
}
...
// Consumes Events from an SQS FIFO queue via long-poll.
...
// Creates a Subscriber that reads from the given SQS queue.
	...
	for {
		...
		for _, msg := range msgs {
			...
			if err := json.Unmarshal([]byte(msg.Body), &evt); err != nil {
				logger.Error(fmt.Sprintf("malformed message id=%s, deleting", msg.ID), err)
				_ = s.sqs.Delete(ctx, s.queueURL, msg.ReceiptHandle)
				continue
			}

			if err := handler(ctx, evt); err != nil {
				logger.Error(fmt.Sprintf("handler failed event=%s id=%s", evt.Type, evt.ID), err)
				// leave in-flight for SQS redrive / DLQ
				continue
			}

			if err := s.sqs.Delete(ctx, s.queueURL, msg.ReceiptHandle); err != nil {
				logger.Error(fmt.Sprintf("failed to delete message event=%s id=%s", evt.Type, evt.ID), err)
			}
		}
	}
}
