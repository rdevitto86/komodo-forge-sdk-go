...
func ContextMiddleware(next http.Handler) http.Handler {
	return http.HandlerFunc(func(wtr http.ResponseWriter, req *http.Request) {
		...
		ctx = context.WithValue(ctx, ctxKeys.VERSION_KEY, helpers.GetAPIVersion(req))
		ctx = context.WithValue(ctx, ctxKeys.URI_KEY, helpers.GetAPIRoute(req))
		ctx = context.WithValue(ctx, ctxKeys.METHOD_KEY, req.Method)
		ctx = context.WithValue(ctx, ctxKeys.PATH_PARAMS_KEY, helpers.GetPathParams(req))
		ctx = context.WithValue(ctx, ctxKeys.QUERY_PARAMS_KEY, helpers.GetQueryParams(req))
		// ctx = context.WithValue(ctx, CLIENT_IP_KEY, utils.GetClientIP(req))
		ctx = context.WithValue(ctx, ctxKeys.CLIENT_TYPE_KEY, helpers.GetClientType(req))
