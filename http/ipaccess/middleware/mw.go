...
import (
	...
	"github.com/rdevitto86/komodo-forge-sdk-go/http/ipaccess"
...
func IPAccessMiddleware(next http.Handler) http.Handler {
	...
	ipOnce.Do(func() {
		wlIPs, wlNets := ipaccess.ParseList(config.GetConfigValue("IP_WHITELIST"))
		blIPs, blNets := ipaccess.ParseList(config.GetConfigValue("IP_BLACKLIST"))
	...
	return http.HandlerFunc(func(wtr http.ResponseWriter, req *http.Request) {
		...
		allowed := ipaccess.Evaluate(ip, &lists)
		if !allowed {
